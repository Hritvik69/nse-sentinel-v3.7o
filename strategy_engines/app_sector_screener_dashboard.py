...
def _cached_index_analysis(index_sector: str) -> dict[str, Any]:
...
def render_sector_screener_dashboard(
    ...
    def _get_market_bias() -> dict | None:
        ...
        if _mb is None and compute_market_bias_fn is not None:
            try:
    ...
    def _run_pipeline(raw_rows: list[dict], market_bias: dict | None = None) -> pd.DataFrame:
        if not raw_rows:
            return pd.DataFrame()
        mb = market_bias if isinstance(market_bias, dict) else _get_market_bias()
        try:
            return _cached_pipeline_df(
    ...
    def _sector_cache_key(
        ...
        return (cache_kind, mode, sector_name, data_signature, _market_bias_key(market_bias))
    ...
    def _build_sector_result(
        ...
        df = _run_pipeline(raw_rows, market_bias=market_bias)
        if df.empty:
            result = {
                "kind": "sector",
                "sector": sector_name,
                ...
                "valid_row_count": 0,
                "err": f"{sector_name} scan returned no valid rows after the full pipeline.",
            }
            return result

        index_analysis = _cached_index_analysis(get_dashboard_index_sector(sector_name))
    ...
    if _active and st.session_state.get("ss_screener_sector_result") is not None:
        ...
        if _err:
            ...
            if _sector_rows:
                ...
                st.dataframe(_sector_df, use_container_width=True, hide_index=True)

            with st.expander(f"📋 All Covered Symbols — {len(_symbols)} unique", expanded=False):
                st.dataframe(pd.DataFrame({"Symbol": _symbols}), use_container_width=True, hide_index=True)

            if _missing:
                with st.expander(f"⚠️ Missing / Skipped Symbols — {len(_missing)}", expanded=False):
                    st.dataframe(pd.DataFrame({"Symbol": _missing}), use_container_width=True, hide_index=True)
