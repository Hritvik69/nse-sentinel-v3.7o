...
try:
    from strategy_engines.sector_master import get_sector_description, SECTOR_DESCRIPTIONS
    _SM_OK = True
except ImportError:
    try:
...
def render_sector_intelligence_section() -> None:
    ...
    if not _SIE_OK:
        st.warning(
            "⚠️ sector_intelligence_engine.py not found. "
        ...
        scan_sig = st.session_state.get("scan_time", None) or f"len={len(_sie_df)}"
